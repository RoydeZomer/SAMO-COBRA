# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 13:05:32 2020
...
with open(r"C:\Users\r.dewinter\Documents\GitHub\SAMO-COBRA\Experiments\results\nsgaii.json") as f:
...
SMSFolder = 'P:/20.xxx Projecten 2020/20.501 PhD Roy de Winter/Support/Python/SAMO-COBRA/sacobra multi/SMS'
...
PHVFolder = 'P:/20.xxx Projecten 2020/20.501 PhD Roy de Winter/Support/Python/sacobra multi - phv/results'
for i in range(len(funcNames)):
    hypPHV = []
    funcName = funcNames[i]
    ref = refs[i]
...
CEGOFolder = 'P:/20.xxx Projecten 2020/20.501 PhD Roy de Winter/Support/Python/CONSTRAINED_EGO/results'
for i in range(len(funcNames)):
    hypCEGO = []
    funcName = funcNames[i]
    ref = refs[i]
...
for funcName in funcNames:
    ...
    if (np.mean(hypPHV) > np.mean(hypSMS) and np.mean(hypPHV) > np.mean(hypCEGO) and np.mean(hypPHV) > np.mean(hypNSII) and np.mean(hypPHV) > np.mean(hypNSIII)):
        ...
        print('PHV SMS', ranksums(hypPHV, hypSMS).pvalue*4)
        print('PHV CEGO', ranksums(hypPHV, hypCEGO).pvalue*4)
        print('PHV NSGAII', ranksums(hypPHV, hypNSII).pvalue*4)
        print('PHV NSGAIII', ranksums(hypPHV, hypNSIII).pvalue*4)
    ...
    if (np.mean(hypSMS) > np.mean(hypPHV) and np.mean(hypSMS) > np.mean(hypCEGO) and np.mean(hypSMS) > np.mean(hypNSII) and np.mean(hypSMS) > np.mean(hypNSIII)):
        #times 4 for bonferoni correction
        print('SMS PHV', ranksums(hypSMS, hypPHV).pvalue*4)
        print('SMS CEGO', ranksums(hypSMS, hypCEGO).pvalue*4)
        print('SMS NSGAII', ranksums(hypSMS, hypNSII).pvalue*4)
        print('SMS NSGAIII', ranksums(hypSMS, hypNSIII).pvalue*4)
    ...
    if (np.mean(hypCEGO) > np.mean(hypPHV) and np.mean(hypCEGO) > np.mean(hypSMS) and np.mean(hypCEGO) > np.mean(hypNSII) and np.mean(hypCEGO) > np.mean(hypNSIII)):
        #times 4 for bonferoni correction
        print('CEGO PHV', ranksums(hypCEGO, hypPHV).pvalue*4)
        print('CEGO SMS', ranksums(hypCEGO, hypSMS).pvalue*4)
        print('CEGO NSGAII', ranksums(hypCEGO, hypNSII).pvalue*4)
        print('CEGO NSGAIII', ranksums(hypCEGO, hypNSIII).pvalue*4)
    ...
    if (np.mean(hypNSII) > np.mean(hypPHV) and np.mean(hypNSII) > np.mean(hypSMS) and np.mean(hypNSII) > np.mean(hypCEGO) and np.mean(hypNSII) > np.mean(hypNSIII)):
        #times 4 for bonferoni correction
        print('NSGAII PHV', ranksums(hypNSII, hypPHV).pvalue*4)
        print('NSGAII SMS', ranksums(hypNSII, hypSMS).pvalue*4)
        print('NSGAII CEGO', ranksums(hypNSII, hypCEGO).pvalue*4)
        print('NSGAII NSGAIII', ranksums(hypNSII, hypNSIII).pvalue*4)
    ...
    if (np.mean(hypNSIII) > np.mean(hypPHV) and np.mean(hypNSIII) > np.mean(hypSMS) and np.mean(hypNSIII) > np.mean(hypCEGO) and np.mean(hypNSIII) > np.mean(hypNSII)):
        #times 4 for bonferoni correction
        print('NSGAIII PHV', ranksums(hypNSIII, hypPHV).pvalue*4)
        print('NSGAIII SMS', ranksums(hypNSIII, hypSMS).pvalue*4)
        print('NSGAIII CEGO', ranksums(hypNSIII, hypCEGO).pvalue*4)
        print('NSGAIII NSGAIII', ranksums(hypNSIII, hypNSII).pvalue*4)
    print()
