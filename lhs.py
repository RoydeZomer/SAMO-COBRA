...
def lhs(n, samples=None, criterion=None, iterations=None, X_start = None):
    """
    ...
    if X_start is not None:
        H = _lhsplusstart(n, samples, X_start)
...
def _lhsplusstart(n, samples, X_start):
    
    
    X_start_orig = X_start
    X_start = np.unique(X_start, axis=0)
    
    samples_xstart, n_xstart = X_start.shape
    if n_xstart != n:
        raise ValueError('The number of parameters in X_start is not equal to the number of parameters requested in the LHS')
    
    # Make the random pairings
    H = np.zeros((samples,n))
    for j in range(n):
        samplevalues = X_start[:,j]
        samplevalues = np.unique(samplevalues)
        samples_xstart = len(samplevalues)
        
        samples_plus = samples+samples_xstart
        
         # Generate the intervals
        cut = np.linspace(0, 1, samples_plus + 1)    
        
        # Fill points uniformly in each interval
        a = cut[:samples_plus]
        b = cut[1:samples_plus + 1]
        _center = (a + b)/2
        
        corrected_center = _center
        for value in samplevalues:
            idx = np.argmin(np.abs(corrected_center-value))
            mask = np.ones(len(corrected_center))==1
            mask[idx] = False
            corrected_center = corrected_center[mask]
        H[:, j] = np.random.permutation(corrected_center)
    
    H = np.vstack((X_start_orig,H))
